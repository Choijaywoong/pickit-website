...
const { handleDemoChat } = require('./demoMode');
 ...
 */
async function chat(userMessage, conversationHistory = [], activeChannels = [], stockMode = null) {
  const apiKey    = cred.get('ANTHROPIC_API_KEY');
  const demoMode  = process.env.DEMO_MODE === 'true' || !apiKey;

  // 데모 모드: 실제 API 없이 목 응답 반환
  if (demoMode) {
    return handleDemoChat(userMessage, activeChannels);
  }
  ...
  // 루프 중 발생한 excel/connectionError/predictions 를 누적 (end_turn 시 최종 응답에 포함)
  let pendingExcel       = null; // { excelData, filename }
  let connectionErrors   = [];   // [{ platform, message, link }, ...]
  let pendingPredictions = [];   // FR-006: [{ productName, option, daysLeft, recommendQty }, ...]
  ...
  while (true) {
    ...
    if (response.stop_reason === 'end_turn') {
      ...
      return {
        ...
        ...(pendingPredictions.length  && { predictions: pendingPredictions }),
      };
    }
    ...
    if (response.stop_reason === 'tool_use') {
      ...
      const result = await handleToolCall({ ...toolUseBlock.input, stockMode });
      ...
      if (result.predictions?.length) {
        pendingPredictions.push(...result.predictions);
      }
